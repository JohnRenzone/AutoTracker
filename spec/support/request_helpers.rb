...
module RequestHelpers
  ...
  def as_user(user=nil, &block)
    current_user = user || FactoryGirl.create(:dealer)
  ...
  def as_admin(user=nil, &block)
    ...
    user.update unless user.admin?
    as_user(user, &block)
  end

  def as_dealer(user=nil, &block)
    user ||= FactoryGirl.create(:dealer)
    user.update unless user.dealer?
    as_user(user, &block)
  end

  def as_service_advisor(user=nil, &block)
    user ||= FactoryGirl.create(:service_advisor)
    user.update unless user.dealer?
    as_user(user, &block)
  end
